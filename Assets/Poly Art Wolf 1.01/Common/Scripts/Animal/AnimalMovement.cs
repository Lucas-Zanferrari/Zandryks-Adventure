...
namespace MalbersAnimations
{
    public partial class Animal
    {
       ...
       protected virtual void LinkingAnimator(Animator anim_)
        {
            if (!death)
            {
        ...
        protected virtual void FixPosition()
        {
            ...
            float distanceHip = 0, distanceChest = 0;
            ...
            float AngleSlope = Vector3.Angle(SurfaceNormal, Vector3.up);                   //Calculate the Angle of the Terrain
            ...
            // slope = ((_Chest.Y - _Hip.Y) / 2) / scaleFactor / maxSlope; //Calculate the Slope Factor (Improve This!!!! by angles)
            ...
            {
                if (slope < 0 || (fall && !isJumping()))      //Don't Align whe is UpHill
                {
        ...
        protected virtual void Swimming()
        {
            ...
            {
                if ((Pivot_Chest.Y < waterlevel && !swim) || (fall && !isJumping(0.5f, true)))
                {
                    swim = true;
                    _rigidbody.constraints = Still_Constraints;
                }
              ...
              else  if (hit_Chest.distance < _Height && !isJumping() && !fall && (backray && swim))
                {
                    swim = false;
                }
            }
        ...
        protected void MovementSystem(float s1, float s2, float s3)
        {
